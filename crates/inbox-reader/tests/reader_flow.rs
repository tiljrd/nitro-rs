...
async fn reader_fetches_delayed_and_batches() {
    ...
    let after_delayed_acc = delayed.msgs[1].after_inbox_acc();
    let sequencer = Arc::new(MockSequencer { after_delayed_acc });
    let headers = Arc::new(MockHeaderReader { number: 10 });
    ...
    let task = tokio::spawn(async move {
        let _ = tokio::time::timeout(std::time::Duration::from_millis(500), reader.start()).await;
    });
    let _ = task.await;

    assert_eq!(tracker.get_delayed_count().unwrap(), 2, "delayed messages stored");
    // assert_eq!(tracker.get_batch_count().unwrap(), 1, "batch stored");
